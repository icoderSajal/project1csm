...
const EditSalonPage = () => {
    ...
    useEffect(() => {
        const fetchSalon = async () => {
            try {
                const res = await axios.get(`http://localhost:8000/api/v1/salon/${id}`, {
                    headers: {
                ...
                if (res.data.success) {
                    ...
                    setForm({
                        ...
                        opening_time: new Date(salon.opening_time).toISOString().substring(11, 16),
                        closing_time: new Date(salon.closing_time).toISOString().substring(11, 16),
                        specialist_haircut: salon.specialist_haircut || "",
                        specialist_manicure: salon.specialist_manicure || "",
                    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...form,
                opening_time: new Date(`1970-01-01T${form.opening_time}`),
                closing_time: new Date(`1970-01-01T${form.closing_time}`),
