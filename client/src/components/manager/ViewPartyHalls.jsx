...
const ViewPartyHalls = () => {
    ...
    const fetchBookings = async () => {
        try {
            ...
            const res = await axios.get("http://localhost:8000/api/v1/party-booking", {
                headers: {
    ...
    return (
        <>
            <div className="p-6">
                <h2 className="text-2xl font-bold mb-4">Party Hall Bookings</h2>
                            ...
                            <tbody>
                                {bookings.map((booking, index) => (
                                    <tr key={index} className="border-b hover:bg-gray-50">
                                        <td className="px-4 py-2">{booking.hall_name}</td>
