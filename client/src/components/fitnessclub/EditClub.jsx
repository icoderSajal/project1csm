...
const EditClub = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/api/v1/fitness/${id}`, {
        headers: {
      ...
      if (response.data.success) {
        setFormData(response.data.club);
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        navigate("/admin-dashboard/clubs"); // Change path if needed
      }
    } catch (error) {
